...
$(HYPERV_BOX_DIR)/$(1)$(BOX_SUFFIX): $(1).json
	...
	$(PACKER) build -only=$(HYPERV_BUILDER) $(PACKER_VARS) -var "iso_url=$(2)" -var "iso_checksum=$(3)" $(1).json
...
$(HYPERV_BOX_DIR)/$(1)-ssh$(BOX_SUFFIX): $(1)-ssh.json
	rm -rf $(HYPERV_OUTPUT)
	mkdir -p $(HYPERV_BOX_DIR)
	$(PACKER) build -only=$(HYPERV_BUILDER) $(PACKER_VARS) -var "iso_url=$(2)" -var "iso_checksum=$(3)" $(1)-ssh.json
...
$(HYPERV_BOX_DIR)/$(1)-cygwin$(BOX_SUFFIX): $(1)-cygwin.json
	rm -rf $(HYPERV_OUTPUT)
	mkdir -p $(HYPERV_BOX_DIR)
	$(PACKER) build -only=$(HYPERV_BUILDER) $(PACKER_VARS) -var "iso_url=$(2)" -var "iso_checksum=$(3)" $(1)-cygwin.json
